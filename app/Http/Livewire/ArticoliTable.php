...
class ArticoliTable extends Component
{
    ...
    // ModalitÃƒÆ’Ã‚Â  modifica articolo
    ...
    // ModalitÃƒÆ’Ã‚Â  gestione immagine articolo
    ...
    // RIMOSSO: Listener JavaScript non piÃƒÆ’Ã‚Â¹ necessario
    ...
    {
        $search = $this->normalizeSearchText((string) $this->magazzinoSearch);
        $grouped = $this->getMagazziniGroupedBySede();

        if ($search === '') {
            return $grouped->map(function ($items) {
                return collect($items)->map(function ($magazzino) {
                    return (object) [
                        'id' => (int) $magazzino->id,
                        'sede_id' => $magazzino->sede_id,
                        'nome' => $this->sanitizeUtf8((string) $magazzino->nome),
                        'codice_locale' => $this->sanitizeUtf8((string) $magazzino->codice_locale),
                    ];
                })->values();
            });
        }
        ...
        return $grouped
            ->map(function ($items) use ($search) {
                return collect($items)->filter(function ($magazzino) use ($search) {
                    return str_contains($this->normalizeSearchText((string) $magazzino->nome), $search)
                        || str_contains($this->normalizeSearchText((string) $magazzino->codice_locale), $search)
                        || str_contains((string) $magazzino->id, $search);
                })->map(function ($magazzino) {
                    return (object) [
                        'id' => (int) $magazzino->id,
                        'sede_id' => $magazzino->sede_id,
                        'nome' => $this->sanitizeUtf8((string) $magazzino->nome),
    ...
    public function getFilteredFornitoriProperty()
    {
        ...
        return Fornitore::where('attivo', true)
            ...
            ->map(function ($fornitore) {
                return (object) [
                    'id' => (int) $fornitore->id,
                    'ragione_sociale' => $this->sanitizeUtf8((string) $fornitore->ragione_sociale),
                ];
            });
    }
    ...
    public function getSelectedMagazziniSummaryProperty(): string
    {
        ...
        if ($labels->count() === 1) {
            return $this->sanitizeUtf8((string) $labels->first());
        }

        return $this->sanitizeUtf8($labels->take(2)->implode(', ') . ' +' . ($labels->count() - 2));
    }
    ...
    public function getFiltriAttiviProperty(): array
    {
        ...
        if (trim((string) $this->search) !== '') {
            $filtri[] = ['field' => 'search', 'label' => 'Ricerca', 'value' => $this->sanitizeUtf8((string) $this->search)];
        }
        ...
        if ($this->fornitoreFilter !== '') {
            ...
            $filtri[] = ['field' => 'fornitore', 'label' => 'Fornitore', 'value' => $this->sanitizeUtf8((string) ($fornitore?->ragione_sociale ?? $this->fornitoreFilter))];
        }

        if ($this->disponibilitaFilter !== '') {
            $filtri[] = ['field' => 'disponibilita', 'label' => 'Disponibilità', 'value' => $this->sanitizeUtf8((string) ($this->disponibilitaOptions[$this->disponibilitaFilter] ?? $this->disponibilitaFilter))];
        }
        ...
        if ($this->ubicazioneFilter !== '') {
            ...
            $filtri[] = ['field' => 'ubicazione', 'label' => 'Ubicazione', 'value' => $this->sanitizeUtf8((string) ($sede?->nome ?? $this->ubicazioneFilter))];
        }
    ...
    public function getResultsSummaryProperty(): string
    {
        ...
        if ($this->disponibilitaFilter !== '') {
            $parts[] = $this->sanitizeUtf8((string) ($this->disponibilitaOptions[$this->disponibilitaFilter] ?? $this->disponibilitaFilter));
        }

        if ($this->fornitoreFilter !== '') {
            $fornitore = Fornitore::find($this->fornitoreFilter);
            if ($fornitore) {
                $parts[] = $this->sanitizeUtf8((string) $fornitore->ragione_sociale);
            }
        }
        ...
        return $this->sanitizeUtf8(implode(' | ', $parts));
    }    
    ...
    public function scaricaArticolo($articoloId)
    {
        try {
            ...
            if ($giacenza == 1) {
                ...
                // Se giacenza > 1, apri modal per scegliere quantitÃƒÆ’Ã‚Â 
    ...
    /**
     * Esegue lo scarico con quantitÃƒÆ’Ã‚Â  specificata
     */
    public function eseguiScarico($articolo, $quantita)
    {
        try {
            ...
            if ($quantita > $giacenzaAttuale) {
                session()->flash('error', 'QuantitÃƒÆ’Ã‚Â  richiesta superiore alla giacenza disponibile');
                return;
            }
    ...
    public function confermaScaricoParziale()
    {
        ...
        if ($this->quantitaDaScaricare > $this->giacenzaDisponibile) {
            session()->flash('error', 'QuantitÃƒÆ’Ã‚Â  superiore alla giacenza disponibile');
            return;
        }
    ...
    /**
     * Apre modal per ricaricare quantitÃƒÆ’Ã‚Â  scaricate per errore
     */
    ...
    /**
     * Conferma ricarico quantitÃƒÆ’Ã‚Â 
     */
    public function confermaRicarico()
    {
        ...
        if ($this->giacenzaMancante !== null && $this->quantitaDaRicaricare > $this->giacenzaMancante) {
            session()->flash('error', 'QuantitÃƒÆ’Ã‚Â  superiore al massimo ripristinabile');
            return;
        }
    ...
    private function sembraPrezzoEuro(string $value): bool
    {
        ...
        $normalized = str_replace('ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬', '', $normalized);
    ...
    private function caricaStampantiDisponibili($articolo)
    {
        ...
        $this->stampantiDisponibili = $stampanti->map(function ($stampante) {
            // Admin/superuser: puÃƒÆ’Ã‚Â² stampare su qualsiasi stampante attiva.
            return [
                'id' => $stampante->id,
                'nome' => $stampante->nome,
        ...
        // Se non c'ÃƒÆ’Ã‚Â¨ una stampante predefinita, seleziona la prima disponibile
    ...
    public function confermaStampaEtichetta()
    {
        ...
        if (empty($this->prezzoEtichetta)) {
            session()->flash('error', 'Il prezzo ÃƒÆ’Ã‚Â¨ obbligatorio');
            return;
        }
    ...
    private function getMagazziniGroupedBySede()
    {
        ...
        $categorie = CategoriaMerceologica::query()
            ...
            ->map(function (CategoriaMerceologica $categoria) use ($service) {
                ...
                return (object) [
                    'id' => (int) $magazzinoLogico,
                    'nome' => $this->sanitizeUtf8((string) $service->getLabelForCategoria($categoria)),
                    'sede_id' => $categoria->sede_id,
                    'sede_nome' => $this->sanitizeUtf8((string) ($categoria->sede->nome ?? ('Sede ' . $categoria->sede_id))),
                    'codice_locale' => $this->sanitizeUtf8((string) $categoria->codice),
                ];
            })
            ->filter();
    ...
    public function render()
    {
        ...
        $articoli->setCollection($this->sanitizeViewValue($articoli->getCollection()));
        $magazzini = $this->sanitizeViewValue($magazzini);
        $magazziniGruppati = $this->sanitizeViewValue($magazziniGruppati);
        $fornitori = $this->sanitizeViewValue($fornitori);
        $marche = $this->sanitizeViewValue($marche);
        $sedi = $this->sanitizeViewValue($sedi);
    ...
    private function sanitizeViewValue($value)
    {
        if (is_string($value)) {
        ...
        if ($value instanceof Collection) {
            return $value->map(fn ($item) => $this->sanitizeViewValue($item));
        }
        ...
        if ($value instanceof Model) {
            ...
            foreach ($attributes as $key => $attributeValue) {
                if (is_string($attributeValue)) {
                    $attributes[$key] = $this->sanitizeUtf8($attributeValue);
                }
            }

            $value->setRawAttributes($attributes, true);

            foreach ($value->getRelations() as $relation => $relationValue) {
                $value->setRelation($relation, $this->sanitizeViewValue($relationValue));
            }
        ...
        if (is_array($value)) {
            foreach ($value as $key => $item) {
                $value[$key] = $this->sanitizeViewValue($item);
            }
        ...
        if (is_object($value)) {
            foreach (get_object_vars($value) as $property => $item) {
                $value->{$property} = $this->sanitizeViewValue($item);
            }
