...
class PomellatoFatturaArticoliParser implements OcrArticoliParser
{
    public function supports(DocumentProfile $profile): bool
    {
        return $profile->is('pomellato_fattura');
    }

    public function parse(string $text, OcrParsingContext $context): array
    {
        return $context->service->parsePomellatoFatturaProfileArticoli($text);
    }
}
