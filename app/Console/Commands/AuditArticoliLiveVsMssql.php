...
class AuditArticoliLiveVsMssql extends Command
{
    protected $signature = 'audit:articoli-live-vs-mssql
                            {--output-dir=audit_articoli_live_vs_mssql : Cartella sotto storage/app per i report}
                            {--only-problematic : Esporta solo mismatch e record senza match MSSQL}
                            {--include-deleted : Include anche articoli soft-deleted}';

    protected $description = 'Confronta gli articoli MySQL con la vista MSSQL live usando numero/data/fornitore effettivi da relazioni';
    ...
    public function handle(): int
    {
        ...
        $root = storage_path('app/' . ($outputDir !== '' ? $outputDir : 'audit_articoli_live_vs_mssql'));
        ...
        if (!$includeDeleted && method_exists(Articolo::class, 'bootSoftDeletes')) {
            $query->whereNull('deleted_at');
        }

        $query->chunkById(500, function ($rows) use (&$exact, &$mismatch, &$missing, &$illegalSplits, $mssqlByBase, $onlyProblematic) {
            foreach ($rows as $articolo) {
                $codice = trim((string) $articolo->codice);
                $baseCode = $this->resolveMysqlBaseCode($codice, $articolo->codice_base);
                $prefix = $this->extractPrefix($codice);
                $mysqlNumero = $this->normalizeDocumentNumber($articolo->numero_documento_carico_effettivo);
                $mysqlData = $this->normalizeDate($articolo->data_carico_effettiva);
                $mysqlFornitoreId = $articolo->fornitore_effettivo_id;
                $mysqlFornitoreName = $articolo->fornitore?->ragione_sociale;

                $payload = [
                    'articolo_id' => (int) $articolo->id,
                    'codice' => $codice,
                    'codice_base' => (string) ($articolo->codice_base ?? ''),
                    'base_code_match' => $baseCode,
                    'prefix' => $prefix,
                    'descrizione_mysql' => trim((string) ($articolo->descrizione ?? '')),
                    ...
                    'deleted_at' => $articolo->deleted_at,
                ];

                if ($this->isIllegalSplit($codice, $prefix, $articolo->codice_base)) {
        ...
        $files['mismatch'] = $this->writeCsv($root, "mismatch_relazionale_{$timestamp}.csv", $mismatch);
        ...
        if (!$onlyProblematic) {
            $files['match'] = $this->writeCsv($root, "match_relazionale_{$timestamp}.csv", $exact);
        }
